...
class DerainCycleGAN(nn.Module):
  ...
  def backward_EG(self, opts):
      ...
      # atten loss
      loss_att_b = self.criterionL2(self.mask_b[5], torch.zeros_like(self.mask_b[5])) * 10
      ...
      self.gan_loss_a = loss_G_GAN_A
      self.gan_loss_b = loss_G_GAN_B
      self.l1_recon_A_loss = loss_G_L1_A
      self.l1_recon_B_loss = loss_G_L1_B
      self.perceptual_loss = loss_perceptual
      self.atten_loss_b = loss_att_b
      self.cons_loss = loss_cons
      self.G_loss = loss_G
  ...
  def assemble_outputs(self, images_a, images_b, masks_a, masks_b):
      # 归一化处理已经在函数外部完成，直接在此处使用传入的图像和掩码

      images_a = self.normalize_image(images_a)
      images_b = self.normalize_image(images_b)
      images_mask_a1 = self.normalize_image(masks_a[0])
      images_mask_a2 = self.normalize_image(masks_a[1])
      images_mask_a3 = self.normalize_image(masks_a[2])
      images_mask_a4 = self.normalize_image(masks_a[3])
      images_mask_a5 = self.normalize_image(masks_a[4])
      images_mask_a6 = self.normalize_image(masks_a[5])

      images_mask_b1 = self.normalize_image(masks_b[0])
      images_mask_b2 = self.normalize_image(masks_b[1])
      images_mask_b3 = self.normalize_image(masks_b[2])
      images_mask_b4 = self.normalize_image(masks_b[3])
      images_mask_b5 = self.normalize_image(masks_b[4])
      images_mask_b6 = self.normalize_image(masks_b[5])

      row1 = torch.cat((images_a[0:1, ::], images_b[0:1, ::]), 3)
      row2 = torch.cat((images_mask_a1[0:1, ::], images_mask_b1[0:1, ::]), 3)
      row3 = torch.cat((images_mask_a2[0:1, ::], images_mask_b2[0:1, ::]), 3)
      row4 = torch.cat((images_mask_a3[0:1, ::], images_mask_b3[0:1, ::]), 3)
      row5 = torch.cat((images_mask_a4[0:1, ::], images_mask_b4[0:1, ::]), 3)
      row6 = torch.cat((images_mask_a5[0:1, ::], images_mask_b5[0:1, ::]), 3)
      row7 = torch.cat((images_mask_a6[0:1, ::], images_mask_b6[0:1, ::]), 3)

      return torch.cat((row1, row2, row3, row4, row5, row6, row7), 2)
